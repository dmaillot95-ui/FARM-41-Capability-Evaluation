...
role=os.environ['ROLE']; space=os.environ['MODEL']; out=pathlib.Path('out'); out.mkdir(exist_ok=True)
registry_context,registry_meta=load_context(['constitution','meta_core','disciplines','keys','banks'])
...
packet={'role':role,'model':space,'inference_success':ok,'status':status,'output':text if ok else '', 'meta':meta,'registry_runtime':registry_meta}
(out/f'{role}.json').write_text(json.dumps(packet,ensure_ascii=False,indent=2),encoding='utf-8')
print(json.dumps({'role':role,'inference_success':ok,'status':status,'registry_runtime':registry_meta},ensure_ascii=False))
